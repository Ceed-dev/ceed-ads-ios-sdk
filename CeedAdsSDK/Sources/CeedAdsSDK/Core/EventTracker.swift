import Foundation

// MARK: - Ceed Ads iOS SDK — Event Tracker
//
// This module handles impression and click tracking.
// It does NOT perform any UI operations.
// Network requests are delegated to APIClient.
//
// Responsibilities:
//  - trackImpression(ad, requestId)
//  - trackClick(ad, requestId)
//
// Both functions construct an EventPayload and send it
// to the backend via APIClient.sendEvent().

final class EventTracker: @unchecked Sendable {

    // MARK: - Thread Safety
    // Protects all mutable state below.
    private let queue = DispatchQueue(label: "com.ceedads.sdk.eventtracker")

    // MARK: - Internal State
    // These values are set once during initialization and reused for every event payload.
    private var appId: String?
    private var conversationId: String?
    private var userId: String?

    private let apiClient: APIClient

    // MARK: - Impression Deduplication
    // Prevents duplicate impression tracking for the same ad + requestId pair.
    private var sentImpressions = Set<String>()

    // MARK: - Init
    init(apiClient: APIClient) {
        self.apiClient = apiClient
    }

    // MARK: - Initialize Tracker
    // Called from SDK.initialize(...)
    // Sets global identifiers for event tracking.
    func initTracker(
        appId: String,
        conversationId: String? = nil,
        userId: String? = nil
    ) {
        queue.sync {
            self.appId = appId
            self.conversationId = conversationId
            self.userId = userId
        }
    }

    // MARK: - Track Impression
    // Sends an impression event when an ad becomes visible.
    // Ensures each ad/requestId pair only sends one impression.
    func trackImpression(
        ad: ResolvedAd,
        requestId: String?
    ) async throws {

        // Snapshot state + perform dedupe under lock (no await while locked).
        let snapshot: (appId: String, conversationId: String?, userId: String?, shouldSend: Bool, requestIdValue: String)
        snapshot = queue.sync {
            guard let appId = self.appId else {
                return ("", nil, nil, false, "")
            }

            let reqId = requestId ?? "unknown"
            let key = "\(ad.id):\(reqId)"

            if self.sentImpressions.contains(key) {
                return (appId, self.conversationId, self.userId, false, reqId)
            }

            self.sentImpressions.insert(key)
            return (appId, self.conversationId, self.userId, true, reqId)
        }

        guard !snapshot.appId.isEmpty else {
            throw CeedAdsError.notInitialized
        }

        guard snapshot.shouldSend else {
            return
        }

        if requestId == nil {
            // MVP limitation: keep behavior aligned with Web SDK
            print("trackImpression: requestId is nil (MVP limitation)")
        }

        let payload = EventPayload(
            type: .impression,
            adId: ad.id,
            advertiserId: ad.advertiserId,
            requestId: snapshot.requestIdValue,
            appId: snapshot.appId,
            conversationId: snapshot.conversationId,
            userId: snapshot.userId
        )

        try await apiClient.sendEvent(payload)
    }

    // MARK: - Track Click
    // Sends a click event when the CTA is tapped.
    func trackClick(
        ad: ResolvedAd,
        requestId: String?
    ) async throws {

        // Snapshot state under lock.
        let snapshot: (appId: String, conversationId: String?, userId: String?, requestIdValue: String)
        snapshot = queue.sync {
            let appId = self.appId ?? ""
            return (appId, self.conversationId, self.userId, requestId ?? "unknown")
        }

        guard !snapshot.appId.isEmpty else {
            throw CeedAdsError.notInitialized
        }

        if requestId == nil {
            // MVP limitation: keep behavior aligned with Web SDK
            print("trackClick: requestId is nil (MVP limitation)")
        }

        let payload = EventPayload(
            type: .click,
            adId: ad.id,
            advertiserId: ad.advertiserId,
            requestId: snapshot.requestIdValue,
            appId: snapshot.appId,
            conversationId: snapshot.conversationId,
            userId: snapshot.userId
        )

        try await apiClient.sendEvent(payload)
    }
}
